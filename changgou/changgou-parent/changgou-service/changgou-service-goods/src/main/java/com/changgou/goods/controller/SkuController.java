...
public class SkuController {
    ...
    Result<List<Sku>> findByStatus(@PathVariable String status){
        ...
        return new Result<List<Sku>>(true, StatusCode.OK,"查询符合条件的sku的列表成功",skuList) ;
    }
    ...
    public Result decrCount(@RequestBody OrderItem orderItem){
        int i = skuService.decrCount(orderItem);
        if(i<=0){
            return new Result(false,StatusCode.ERROR,"减少库存失败");
        }
        return new Result(true,StatusCode.OK,"更新减少库存成功");
    }
}
