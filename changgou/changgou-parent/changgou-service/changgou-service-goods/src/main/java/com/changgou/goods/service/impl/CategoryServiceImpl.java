...
public class CategoryServiceImpl implements CategoryService {
    ...
    public List<Category> findByParentId(Integer pid) {
        // SELECT * from tb_category where parent_id= 0;  注意下 :当parent_id=0 表示一级分类
        Category condition = new Category();
        condition.setParentId(pid);// 对象中有个属性parent_id 的值为pid  ---->作为条件传递  where parent_id= 0;
        //condition.setName("abc");// where parent_id= 0 and name = "abc"
        return categoryMapper.select(condition);//condition 就是查询的条件 (只能是等号) SELECT * from tb_category
