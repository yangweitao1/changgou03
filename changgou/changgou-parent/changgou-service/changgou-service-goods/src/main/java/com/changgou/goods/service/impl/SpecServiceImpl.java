...
public class SpecServiceImpl implements SpecService {
    ...
    public List<Spec> findByCategoryId(Integer id) {
        //1.根据商品的分类的ID 获取商品分类的对象数据(对象数据有模板的ID)
        Category category = categoryMapper.selectByPrimaryKey(id);
        //2.根据模板的ID 从规格表中获取规格的列表数据  select * from tb_spec where template_id=42
        Spec condition = new Spec();
        condition.setTemplateId(category.getTemplateId());// where template_id=42
        List<Spec> select = specMapper.select(condition);//select * from tb_spec
        //3.返回
        return select;
    }
}
